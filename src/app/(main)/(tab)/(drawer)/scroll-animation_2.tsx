...
const HomeScreen = () => {
  ...
  const cardStyle = useAnimatedStyle(() => {
    const borderRadius = interpolate(scrollY.value, [0, 80], [24, 0], Extrapolate.CLAMP);
    return {
          ...
          <SafeAreaView edges={['top']}>
            ...
            <View style={styles.filterContainer}>
              <TouchableOpacity style={styles.filterButton}>
                ...
                <FeatherIcons name='chevron-down' size={18} style={styles.filterButtonIcon} />
              </TouchableOpacity>
            </View>
          </SafeAreaView>
        )}
        ...
        renderItem={({ item }) => (
          ...
          >
            <Animated.View style={[styles.card, { backgroundColor: item.color }, cardStyle]}>
              <View style={[styles.cardCircle, { backgroundColor: item.fgColor }]} />
              <Text style={[styles.cardTitle, { color: item.fgColor }]}>Current Balance</Text>
              <Text style={[styles.cardPriceText, { color: item.fgColor }]}>${item.balance.toLocaleString()}</Text>
            </Animated.View>
          </Animated.View>
        )}
      />
